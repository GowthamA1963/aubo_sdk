...
robot_ip = "192.168.0.141"  # Robot arm IP address
...
send a second point at the 5th second to rotate joint 3 by 90 degrees.
...
def example_servoj(rpc_client):
    ...
    while not robot_interface.getMotionControl().isServoModeEnabled():
        if i > 5:
            print("Failed to enable Servo mode! Current Servo status: ",
    ...
    while robot_interface.getMotionControl().isServoModeEnabled():
        if i > 5:
            print("Failed to disable Servo mode! Current Servo mode: ",
                  robot_interface.getMotionControl().isServoModeEnabled())
            return -1
        time.sleep(0.005)
        i += 1
